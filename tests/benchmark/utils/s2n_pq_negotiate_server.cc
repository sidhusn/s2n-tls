...
#include "tests/benchmark/utils/s2n_pq_negotiate_server.h"
...
static int benchmark_pq_single_suite_server(benchmark::State& state) {
...
int start_pq_benchmark_server(int argc, char **argv) {
    const char *cipher_prefs_r3 = "PQ-TLS-1-0-2021-05-23";
    ...
    s2n_set_common_server_config(max_early_data, config, conn_settings, cipher_prefs_r3, session_ticket_key_file_path);
    ...
    while ((!stop_listen) && (connectionfd = accept(sockfd, ai->ai_addr, &ai->ai_addrlen)) > 0) {
        for (long int suite_num = 0; suite_num < 3; ++suite_num) {
            for(int i = 0; i < 10; i++) {
                ...
                benchmark::RegisterBenchmark(bench_name.c_str(), benchmark_pq_single_suite_server)
                    ->Repetitions(repetitions)
                    ->Iterations(iterations)
                    ->Args({ suite_num, warmup_iters, connectionfd, i});
            }
        }
        stop_listen = true;
    }
