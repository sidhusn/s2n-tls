...
#include "tests/benchmark/utils/s2n_pq_negotiate_client.h"
...
static void client_handshake(benchmark::State& state, bool warmup, struct s2n_connection *conn, int sockfd) {
    ...
    if(warmup) {
        //printf("KEM: %s KEM_GROUP: %s\n", s2n_connection_get_kem_name(conn), s2n_connection_get_kem_group_name(conn));
    }
    if (DEBUG_PRINT) {
...
static void benchmark_pq_single_suite_client(benchmark::State& state) {
...
/*
    const struct s2n_security_policy security_policy_benchmark_r2 = {
        S2N_SSLv3,
...
*/
    const struct s2n_kem_group *kem_test_groups_list[] = {
    ...
    const struct s2n_kem_group *kem_test_groups[] = {
        kem_test_groups_list[state.range(3)]
    };

    const struct s2n_kem_preferences kem_test_prefs = {
        0,
        NULL,
        s2n_array_len(kem_test_groups),
        kem_test_groups,
    };

    const struct s2n_security_policy security_policy_benchmark_r3 = { //PQ-TLS-1-0-2021-05-26
        S2N_TLS10,
        &cipher_preferences_benchmark,
        &kem_test_prefs,
        &s2n_signature_preferences_20200207,
        NULL,
        &s2n_ecc_preferences_20200310,
    };

    config->security_policy = &security_policy_benchmark_r3;
...
int start_pq_benchmark_client(int argc, char** argv) {
    all_suites = cipher_preferences_pq_tls_1_0_2021_05_23.suites;
    num_suites = cipher_preferences_pq_tls_1_0_2021_05_23.count;
    int use_corked_io = 0;
    int insecure = 0;
    int sockfd = 0;
    conn_settings = {0};
    ...
    for (long int current_suite = 0; current_suite < 3; current_suite++) {
        for(int i = 0; i < 10; i++) {
            std::string bench_name = std::string("Client: ") + all_suites[current_suite]->name;
            std::string kem_name = kem_test_groups_list[i]->name;
            bench_name = bench_name + " " + kem_name;
            benchmark::RegisterBenchmark(bench_name.c_str(), benchmark_pq_single_suite_client)
                ->Repetitions(repetitions)
                ->ReportAggregatesOnly()
                ->Iterations(iterations)
                ->Args({ current_suite, warmup_iters, sockfd, i});
        }
    }
